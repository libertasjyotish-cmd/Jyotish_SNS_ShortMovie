...
## 5. 週間配信スケジュール（16 本/週・言語）

| 曜日 | 枠 | 対象 | 本数 | 投稿時刻 (JST) |
| --- | --- | --- | --- | --- |
| 月〜木 | テーマ動画（`Evergreen_Scripts`） | 全星座共通 | 各 1 | 18:00 |
| 金（前週） | 月星座 weekly | 牡羊/牡牛/双子/蟹 | 4 | 12:00 / 15:00 / 18:00 / 21:00 |
| 土（前週） | 月星座 weekly | 獅子/乙女/天秤/蠍 | 4 | 12:00 / 15:00 / 18:00 / 21:00 |
| 日（前週） | 月星座 weekly | 射手/山羊/水瓶/魚 | 4 | 12:00 / 15:00 / 18:00 / 21:00 |

月星座 weekly は対象週の前週 金〜日に投稿する（`zodiacPostWeekStart`）。対象週に入ってから
出すと「今週の運勢」が読まれる前に終わってしまうため、視聴者が予定を立てられる週末に前出しする。

同日の 4 本は同時刻に出すと自分の投稿同士で初速を食い合うため、`ZODIAC_SLOTS_JST` の
3 時間刻みの枠へ順に割り当てる。`daily-dispatch` は 12/15/18/21 JST（`0 3,6,9,12 * * *` UTC）に走り、
`scheduled_post_time` を過ぎた未投稿タスクだけを投稿する。

プラン作成（`weekly-plan`）は火 02:00 JST、原稿生成（`weekly-generate`）は火 03:00 JST、
レンダリング（`render-batch`）は火 05:00〜木 22:00 JST に分割実行。前出しした月星座 weekly の
初回投稿（金 12:00 JST）までにレンダリングが終わるよう、対象週の前週の頭に動かしている。
